...
#endif
$output v_bitangent, v_color0, v_normal, v_tangent, v_texcoord0, v_viewDir, v_wpos
struct NoopSampler {
    int noop;
};
...
void VertShared(VertexInput vertInput, inout VertexOutput vertOutput) {
    ...
    vec3 wnormal = ((World) * (vec4(normal.xyz, 0.0))).xyz; // Attention!
    vec3 tangent = vertInput.tangent.xyz;
    vec3 wtangent = ((World) * (vec4(tangent.xyz, 0.0))).xyz; // Attention!
    vec3 viewNormal = normalize(((View) * (vec4(wnormal, 0.0))).xyz); // Attention!
    vec3 viewTangent = normalize(((View) * (vec4(wtangent, 0.0))).xyz); // Attention!
    ...
    vec3 view = ((View) * (vec4(wpos, 0.0))).xyz; // Attention!
    vertOutput.viewDir = ((view) * (tbn)); // Attention!
    ...
    vertOutput.texcoord0 = ((UV0Transform) * (vec4(vertOutput.texcoord0, 0, 1))).xy; // Attention!
    #endif
