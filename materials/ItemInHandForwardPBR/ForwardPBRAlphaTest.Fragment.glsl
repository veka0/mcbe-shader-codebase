...
int var_e7b23;
float var_33fae;
layout(binding = 10, std430) buffer s_zVoxelBuffer { VoxelNode zVoxelBuffer[]; } var_1557a;
layout(binding = 7, std430) buffer s_zGpuEntryBuffer { GpuVolumeEntry zGpuEntryBuffer[]; } var_76553;
...
void func_f73f0(inout uint arg_a6daa, inout highp vec3 arg_aa7d7) {
    if (var_1557a.zVoxelBuffer[arg_a6daa].data == 0u)
    {
        arg_aa7d7 = vec3(0.0);
        return;
    }
    highp vec4 loc_96d01 = vec4(uvec4(var_1557a.zVoxelBuffer[arg_a6daa].data, var_1557a.zVoxelBuffer[arg_a6daa].data >> 8u, var_1557a.zVoxelBuffer[arg_a6daa].data >> 16u, var_1557a.zVoxelBuffer[arg_a6daa].data >> 24u) & uvec4(255u)) * vec4(0.0039215688593685626983642578125);
...
void func_968b0(inout highp vec3 arg_cbb88, inout highp vec3 arg_951a8, inout uint arg_b5a6d, inout highp vec3 arg_cfabd) {
    ...
    if (!((var_1557a.zVoxelBuffer[arg_b5a6d + 4096u].data & (1u << uint(var_d93d2[loc_1a9f5]))) != 0u))
    {
        arg_cfabd = vec3(0.0);
        return;
    }
    uvec3 loc_27255 = uvec3(arg_cbb88 - (floor(arg_cbb88 * 0.0625) * 16.0)) & uvec3(15u);
    uint loc_8f6af = (var_1557a.zVoxelBuffer[(arg_b5a6d + 4097u) + uint(var_d93d2[loc_1a9f5])].data >> 2u) + ((loc_27255.y + (loc_27255.z * 16u)) + (loc_27255.x * 256u));
...
void main() {
    ...
    if (DirectionalLightToggleAndMaxDistanceAndMaxCascadesPerLightAndGPUBlockLightingEnabled.w != 0.0)
    {
        ...
        for (;;)
        {
            if (var_d6224 < 8)
            {
                uint var_23c71 = uint(var_76553.zGpuEntryBuffer[var_bcfea].hash) & 65535u;
                bool var_734de = var_23c71 == var_19109;
                bool var_166d2;
                if (var_734de)
                {
                    var_166d2 = var_76553.zGpuEntryBuffer[var_bcfea].packed_xy == var_ef361;
                }
                else
                {
                    var_166d2 = var_734de;
                ...
                if (var_166d2)
                {
                    var_09802 = var_76553.zGpuEntryBuffer[var_bcfea].packed_zw == var_10f25;
                }
                else
                {
                    var_09802 = var_166d2;
                ...
                else
                {
                    ...
                    if (var_09802)
                    {
                        var_c38b2 = var_76553.zGpuEntryBuffer[var_bcfea].user_data;
                    }
                    else
                    {
                        var_c38b2 = var_f503c;
