...
float var_33fae;
layout(binding = 1, std430) buffer s_PBRData { PBRTextureData PBRData[]; } var_b7e16;
...
void func_89801(inout highp float arg_6a625, inout highp float arg_9eee0, inout highp float arg_a50e1, inout highp float arg_d2a5b, inout highp vec3 arg_2dec6) {
    ...
    highp vec2 loc_e8046 = vec2(var_b7e16.PBRData[v_pbrTextureId].colourToNormalUvScale0, var_b7e16.PBRData[v_pbrTextureId].colourToNormalUvScale1);
    highp vec2 loc_3128d = vec2(var_b7e16.PBRData[v_pbrTextureId].colourToNormalUvBias0, var_b7e16.PBRData[v_pbrTextureId].colourToNormalUvBias1);
    highp vec3 loc_b4ff6;
    if ((var_b7e16.PBRData[v_pbrTextureId].flags & 4) == 4)
    {
    ...
    else
    {
        ...
        if ((var_b7e16.PBRData[v_pbrTextureId].flags & 8) == 8)
        {
            highp vec2 loc_218fe = (v_texcoord0 * loc_e8046) + loc_3128d;
            highp vec3 loc_2ae5f = vec3(0.0, 0.0, 1.0);
            highp float loc_b88fd = clamp((min(var_b7e16.PBRData[v_pbrTextureId].maxMipNormal - var_b7e16.PBRData[v_pbrTextureId].maxMipColour, var_b7e16.PBRData[v_pbrTextureId].maxMipNormal) * (-1.0)) + 2.0, 0.0, 1.0);
            if (loc_b88fd > 0.0)
            {
        ...
        else
        {
            ...
            if ((var_b7e16.PBRData[v_pbrTextureId].flags & 16) == 16)
            {
                highp vec2 loc_268f9 = (v_texcoord0 * loc_e8046) + loc_3128d;
                highp float loc_d849b = min(var_b7e16.PBRData[v_pbrTextureId].maxMipNormal - var_b7e16.PBRData[v_pbrTextureId].maxMipColour, var_b7e16.PBRData[v_pbrTextureId].maxMipNormal);
    ...
    if ((var_b7e16.PBRData[v_pbrTextureId].flags & 1) == 1)
    {
        highp vec4 loc_300fb = texture(s_MatTexture, (v_texcoord0 * vec2(var_b7e16.PBRData[v_pbrTextureId].colourToMaterialUvScale0, var_b7e16.PBRData[v_pbrTextureId].colourToMaterialUvScale1)) + vec2(var_b7e16.PBRData[v_pbrTextureId].colourToMaterialUvBias0, var_b7e16.PBRData[v_pbrTextureId].colourToMaterialUvBias1));
        highp float loc_c4db1;
        if ((var_b7e16.PBRData[v_pbrTextureId].flags & 2) == 2)
        {
            loc_c4db1 = loc_300fb.w;
        }
        else
        {
            loc_c4db1 = var_b7e16.PBRData[v_pbrTextureId].uniformSubsurface;
        }
        loc_d7d8a = loc_c4db1;
    ...
    {
        loc_d7d8a = var_b7e16.PBRData[v_pbrTextureId].uniformSubsurface;
        loc_00c14 = var_b7e16.PBRData[v_pbrTextureId].uniformEmissive;
        loc_73c14 = var_b7e16.PBRData[v_pbrTextureId].uniformMetalness;
        loc_659d6 = var_b7e16.PBRData[v_pbrTextureId].uniformRoughness;
    }
...
void main() {
    ...
    highp vec4 var_22a3f = vec4(var_f9ddb.x, var_f9ddb.y, var_f9ddb.z, var_0d9b4.w);
#endif
#ifdef MULTI_COLOR_TINT__OFF
    ...
    highp vec4 var_22a3f = vec4(var_99f3c.x, var_99f3c.y, var_99f3c.z, var_0d9b4.w);
#endif
    ...
    func_89801(var_5431f, var_08af3, var_de0d6, var_bd3b6, var_d2ce2);
    highp vec4 var_08b04 = vec4(var_22a3f.xyz, var_22a3f.w);
    ...
    if ((((var_ec82b.x + var_ec82b.y) + var_ec82b.z) < 9.9999997473787516355514526367188e-05) && (TileLightIntensity.x > 9.9999997473787516355514526367188e-05))
    {
        ...
        var_eb137 = clamp(vec3(var_c1e8f, var_c1e8f * ((((var_c1e8f * 0.60000002384185791015625) + 0.4000000059604644775390625) * 0.60000002384185791015625) + 0.4000000059604644775390625), var_c1e8f * (((var_c1e8f * var_c1e8f) * 0.60000002384185791015625) + 0.4000000059604644775390625)), vec3(0.0), vec3(1.0));
    }
    else
    {
        var_eb137 = var_d13a4;
    ...
    uvec2 var_180e3 = uvec2(uint(clamp(var_08af3, 0.0, 1.0) * 255.0) & 255u, uint(clamp(var_de0d6, 0.0, 1.0) * 255.0) & 255u);
