...
void func_b4bb5(inout highp vec3 arg_534d1, inout highp vec3 arg_90b60, inout highp vec3 arg_218ea, inout highp vec3 arg_016ce, inout highp vec3 arg_68e47, inout highp vec3 arg_470f5, inout highp vec2 arg_f36e7, inout highp vec3 arg_81f79, inout highp vec3 arg_58ffc, inout highp vec3 arg_28cf7, inout highp float arg_cdb42) {
    ...
    for (int loc_e68b0 = 0; loc_e68b0 < loc_a14ca; loc_0bf4d = loc_3ac48, loc_9d63a = loc_f1b97, loc_7a912 = loc_f5dfb, loc_bd26f = loc_1a85e, loc_e68b0++)
    {
        ...
        loc_f1b97 = loc_9d63a + (((((((vec3(1.0) - loc_d5257) * mix(loc_1e1bf, max((dot(arg_218ea, loc_921fd) + SubsurfaceScatteringContributionAndDiffuseWrapValueAndFalloffScale.y) / (loc_2d61b * loc_2d61b), 0.0), arg_cdb42)) * (loc_97c67 * vec3(0.3183098733425140380859375))) * loc_f1dc4) + (((loc_97c67 * vec3(0.3183098733425140380859375)) * (arg_cdb42 * max((dot(-arg_218ea, loc_921fd) + SubsurfaceScatteringContributionAndDiffuseWrapValueAndFalloffScale.y) / (loc_c20a0 * loc_c20a0), 0.0))) * loc_a416a)) * loc_49071) * DiffuseSpecularEmissiveAmbientTermToggles.x);
...
void main() {
    ...
    highp vec3 var_0e102 = var_9f386.xyz;
    ...
    highp float var_106e2 = length(var_0e102);
    ...
    if (var_5bd0a.z != 1.0)
    {
        highp vec3 var_6ce89;
        if (QuantizationParameters.y > 0.0)
        {
            highp vec3 var_d3fcd = v_worldPos - WorldOrigin.xyz;
            highp vec3 var_2c114 = normalize(round(normalize((u_invView * vec4(normalize(cross(normalize(dFdx(var_0e102)), normalize(dFdy(var_0e102)))), 0.0)).xyz) / vec3(QuantizationPrecisionRoundingParameters.x)) * QuantizationPrecisionRoundingParameters.x);
            highp vec3 var_cbc5a = mod(var_d3fcd, vec3(QuantizationParameters.z));
            var_6ce89 = (round((var_d3fcd - (var_cbc5a - (var_2c114 * dot(var_cbc5a, var_2c114)))) / vec3(QuantizationPrecisionRoundingParameters.y)) * QuantizationPrecisionRoundingParameters.y) + WorldOrigin.xyz;
        }
        else
        {
            var_6ce89 = v_worldPos;
        }
        highp vec3 var_aded7 = -(var_0e102 / vec3(length(var_0e102) + 9.9999997473787516355514526367188e-05));
        highp float var_76cac = MERSUniforms.w * SubsurfaceScatteringContributionAndDiffuseWrapValueAndFalloffScale.x;
        highp vec3 var_d5595 = var_0e102;
        highp vec2 var_24506 = vec2(var_30c68, var_ba9ef);
        highp vec3 var_2a503;
        highp vec3 var_8efb6;
        func_b4bb5(var_8efb6, var_2a503, var_56916, var_6ce89, var_7c768, var_d5595, var_24506, var_aded7, var_51c68, var_f7158, var_76cac);
        highp vec3 var_850d6 = var_0e102;
        ...
        highp vec3 var_a69d6 = var_0e102;
        highp vec4 var_05431;
        highp vec3 var_dab42;
        highp vec3 var_9bfc3;
        func_3a820(var_a54cc, var_36c8d, var_9bfc3, var_dab42, var_05431, var_a69d6, var_49c5c, var_0e102, var_56916, var_aded7, var_51c68, var_f7158, var_76cac, var_6ce89, var_7c768);
    ...
    {
        highp float var_79b3e = clamp((((length(var_0e102) / FogAndDistanceControl.z) + RenderChunkFogAlpha.x) - FogAndDistanceControl.x) * FogAndDistanceControl.y, 0.0, 1.0);
    ...
    if (IBLParameters.x != 0.0)
    {
        ...
        if (QuantizationParameters.w > 0.0)
        {
            ...
            highp vec3 var_df277 = normalize(round(normalize((u_invView * vec4(normalize(cross(normalize(dFdx(var_0e102)), normalize(dFdy(var_0e102)))), 0.0)).xyz) / vec3(QuantizationPrecisionRoundingParameters.x)) * QuantizationPrecisionRoundingParameters.x);
            ...
            var_d11db = (u_view * vec4(var_7b993, 1.0)).xyz;
            var_a8715 = var_7b993;
        }
        else
        {
    ...
    else
    {
        ...
        if (DiffuseSpecularEmissiveAmbientTermToggles.w != 0.0)
        {
            highp vec3 var_7751d;
            if (QuantizationParameters.w > 0.0)
            {
                highp vec3 var_437f1 = v_worldPos - WorldOrigin.xyz;
                highp vec3 var_3c740 = normalize(round(normalize((u_invView * vec4(normalize(cross(normalize(dFdx(var_0e102)), normalize(dFdy(var_0e102)))), 0.0)).xyz) / vec3(QuantizationPrecisionRoundingParameters.x)) * QuantizationPrecisionRoundingParameters.x);
                highp vec3 var_377ae = mod(var_437f1, vec3(QuantizationParameters.z));
                var_7751d = (u_view * vec4((round((var_437f1 - (var_377ae - (var_3c740 * dot(var_377ae, var_3c740)))) / vec3(QuantizationPrecisionRoundingParameters.y)) * QuantizationPrecisionRoundingParameters.y) + WorldOrigin.xyz, 1.0)).xyz;
            }
            else
            {
                var_7751d = var_0e102;
            ...
            highp vec2 var_c1068 = vec2(clamp(dot(var_56916, -normalize(var_7751d)), 0.0, 1.0), MERSUniforms.z);
