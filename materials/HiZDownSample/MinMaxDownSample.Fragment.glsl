...
void main() {
    highp vec2 var_85f8a = floor(v_texcoord0 * MipResolutionAndRecipResolution.xy) * MipResolutionAndRecipResolution.zw;
    highp vec4 var_821d7 = vec4(0.0);
#ifdef MODE__MIP
    highp vec4 var_6068e = vec4(0.0);
#endif
    highp vec2 var_975ea = texture(s_PreviousMip, var_85f8a).xy;
    var_821d7.x = var_975ea.x;
#ifdef MODE__MIP
    var_6068e.x = var_975ea.y;
#endif
    var_975ea = texture(s_PreviousMip, var_85f8a + (vec2(1.0, 0.0) * PreviousMipResolutionAndRecipResolution.zw)).xy;
    var_821d7.y = var_975ea.x;
#ifdef MODE__MIP
    var_6068e.y = var_975ea.y;
#endif
    var_975ea = texture(s_PreviousMip, var_85f8a + PreviousMipResolutionAndRecipResolution.zw).xy;
    var_821d7.z = var_975ea.x;
#ifdef MODE__MIP
    var_6068e.z = var_975ea.y;
#endif
    var_975ea = texture(s_PreviousMip, var_85f8a + (vec2(0.0, 1.0) * PreviousMipResolutionAndRecipResolution.zw)).xy;
