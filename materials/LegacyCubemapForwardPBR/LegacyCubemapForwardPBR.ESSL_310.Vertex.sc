...
void VertForwardPBRTransparent(StandardVertexInput vertInput, inout VertexOutput vertOutput) {
    vertOutput.worldPos = ((World) * (((CubemapRotation) * (vec4(vertInput.vertInput.position, 1.0))))).xyz; // Attention!
    vertOutput.position = jitterVertexPosition(vertOutput.worldPos);
    vertOutput.clipPosition = vertOutput.position;
}
