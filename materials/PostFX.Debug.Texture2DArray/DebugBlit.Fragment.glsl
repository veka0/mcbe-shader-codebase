...
void main() {
    highp vec4 var_7a4cf = texture(s_RasterColor, vec3(v_texcoord0, TextureArrayIndex.x));
    highp vec4 var_84c0b = var_7a4cf;
    bool var_fedba;
    ...
    switch (uint(DebugMode.x))
    {
        case 1u:
        {
            var_14810 = var_7a4cf.www;
            var_fedba = false;
            break;
        }
        case 2u:
        {
            highp vec2 var_7bee0 = (var_7a4cf.xy * 2.0) - vec2(1.0);
        ...
        case 3u:
        {
            highp vec2 var_3b7a3 = var_7a4cf.xy;
            highp vec3 var_4ad52 = vec3(var_7a4cf.xy, (1.0 - abs(var_3b7a3.x)) - abs(var_3b7a3.y));
        ...
        case 4u:
        {
            var_14810 = vec3(((clamp(var_7a4cf.xy, vec2(-1.0), vec2(1.0)) * 10.0) * 0.5) + vec2(0.5), 0.0);
            var_fedba = false;
            break;
        }
        case 5u:
        {
            var_14810 = vec3(((clamp(var_7a4cf.zw, vec2(-1.0), vec2(1.0)) * 10.0) * 0.5) + vec2(0.5), 0.0);
            var_fedba = false;
            break;
        }
        case 6u:
        {
            var_14810 = vec3(var_7a4cf.xy, 0.0);
            var_fedba = false;
            break;
        }
        case 7u:
        {
            var_14810 = (var_7a4cf.xyz * 0.0500000007450580596923828125) + vec3(0.5);
            var_fedba = false;
            break;
        }
        case 8u:
        {
            var_14810 = var_7a4cf.xyz;
            var_fedba = false;
            break;
        }
        case 9u:
        {
            var_14810 = var_7a4cf.xxx;
            var_fedba = false;
            break;
        }
        case 10u:
        {
            var_14810 = var_7a4cf.yyy;
            var_fedba = false;
            break;
        }
        case 11u:
        {
            var_14810 = var_7a4cf.zzz;
            var_fedba = false;
            break;
        }
        case 12u:
        ...
        case 13u:
        {
            highp float var_eda2e = clamp((log2(dot(var_7a4cf.xyz, vec3(0.2125000059604644775390625, 0.7153999805450439453125, 0.07209999859333038330078125)) * 8.0) + 4.0) * 0.5, 0.0, 8.0);
        ...
        case 21u:
        {
            var_14810 = vec3(((vec2(ClipPlanes.x * ClipPlanes.y) / (vec2(ClipPlanes.y) + (var_7a4cf.xy * (ClipPlanes.x - ClipPlanes.y)))) - vec2(ClipPlanes.x)) / vec2(ClipPlanes.y - ClipPlanes.x), 0.0);
            var_fedba = true;
            break;
        }
        default:
