...
void main() {
    ...
    highp vec3 var_7d238 = var_66629.xyz;
    highp float var_29db5 = log2(clamp(dot(var_7d238, vec3(0.2125999927520751953125, 0.715200006961822509765625, 0.072200000286102294921875)), LuminanceRangeParams.z, LuminanceRangeParams.w)) - var_39afe;
    highp vec4 var_6e2c7 = texture(s_DownsampledLogLuminance, v_texcoord0);
    highp float var_abf2c = var_6e2c7.x - var_39afe;
    highp vec2 var_0d22a = texture(s_FilteredBilateralGrid, vec3(v_texcoord0, (var_29db5 - LuminanceRangeParams.x) / (LuminanceRangeParams.y - LuminanceRangeParams.x))).xy;
    highp float var_fef48;
    ...
    {
        var_fef48 = mix(LuminanceRangeParams.x + ((var_0d22a.y / var_0d22a.x) * (LuminanceRangeParams.y - LuminanceRangeParams.x)), var_abf2c, LocalExposureParams.z);
    }
    highp float var_fa93d = log2(clamp(texture(s_AverageLuminance, vec2(0.5)).x, LuminanceRangeParams.z, LuminanceRangeParams.w));
    bgfx_FragColor = vec4(var_7d238 * exp2(((var_fa93d + (LocalExposureParams.x * (var_fef48 - var_fa93d))) + (LocalExposureParams.y * (var_29db5 - var_fef48))) - var_29db5), 1.0);
}
