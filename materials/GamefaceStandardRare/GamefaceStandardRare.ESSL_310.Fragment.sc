...
void ShadeGeometry(in FragmentInput fragInput, inout vec4 outColor, inout float alpha) {
    if (int(ShaderType.x) == 1) {
        ...
        for(int i = 0; i < stepsCount; ++ i) {
            float coeff = Coefficients[i / 4][int(mod(float(i), 4.0))]; // Attention!
            vec2 offset;
            offset.x = PixelOffsets[(i * 2) / 4][int(mod(float(i * 2), 4.0))]; // Attention!
            offset.y = PixelOffsets[(i * 2 + 1) / 4][int(mod(float(i * 2 + 1), 4.0))]; // Attention!
        ...
        vec3 rgb = ((yuv2rgb) * (YCbCr)); // Attention!
