...
void Frag(FragmentInput fragInput, inout FragmentOutput fragOutput) {
    #ifdef OPAQUE_PASS
    vec4 color = fragInput.color;
    color.rgb = mix(color, OverlayColor, OverlayColor.a).rgb;
    color *= fragInput.light;
    color.rgb = applyFogVanilla(color.rgb, fragInput.fog.rgb, fragInput.fog.a);
    fragOutput.Color0 = color;
    #endif
    #ifdef TRANSPARENT_PASS
    vec4 diffuse = textureSample(s_BeaconTexture, fragInput.texCoords);
    diffuse.rgb = mix(diffuse, OverlayColor, OverlayColor.a).rgb;
    diffuse *= fragInput.color;
