...
#endif
$output v_color0, v_texcoord0, v_viewSpaceNormal
#ifndef CUSTOM_PASS_BASED_ON_OPAQUE_PASS
$output v_viewSpacePosition
#endif
$output v_worldPos
struct NoopSampler {
    int noop;
...
void computeLighting_BlinnPhong_Vertex(VertexInput vInput, inout VertexOutput vOutput, vec3 worldPosition) {
    ...
    vec3 viewSpaceNormal = ((WorldView) * (vec4(objectNormal.xyz, 0.0))).xyz; // Attention!
    vOutput.viewSpaceNormal = viewSpaceNormal;
    vOutput.viewSpacePosition = ((View) * (vec4(worldPosition.x, worldPosition.y, worldPosition.z, 1.0))); // Attention!
}
#endif
#ifndef DEPTH_ONLY_PASS
